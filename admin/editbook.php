...
<body>
    ...
    <div class="mbook">
        <p>Edit Book</p>
        <?php
        include "toeditbooks.php";
        ?>
        


        <form id="editBookForm" method="post" action="updatebook.php">
            <input type="hidden" name="bookId" value="<?php echo $bookId; ?>">

            <div class="edit1">
                <label>Book Image:</label>
                <img src="uploads/<?php echo $bookDetails['bookimg']; ?>" alt="Book Image" style="width: 130px; height: 170px;">
            </div>

            <div class="edit2">
                <label for="bookName" style="margin-top: 100px;">Book Name:</label><br>
                <input type="text" name="bookName" value="<?php echo $bookDetails['bookName']; ?>" required>
            </div>
            ...
            <div class="edit3">
                ...
                <?php
                $authorsQuery = "SELECT * FROM tbautor";
                $authorsResult = mysqli_query($db, $authorsQuery);
                ?>

                <select class="edit" name="bookAuthors">
                    <option value="" disabled required>Select Author</option>
                    <?php
                    while ($r = mysqli_fetch_array($authorsResult)) {
                        $selected = ($bookDetails['bookAuthors'] == $r['authorname']) ? 'selected' : '';
                        ?>
                        <option value="<?php echo $r['authorname']; ?>" <?php echo $selected; ?>><?php echo $r['authorname']; ?></option>
                    <?php
                    }
                    ?>
                </select>
            ...
            <div class="edit4">
                ...
                <?php
                $genresQuery = "SELECT * FROM tbgenre";
                $genresResult = mysqli_query($db, $genresQuery);
                ?>
                <select class="edit" name="bookCategory">
                    <option value="" disabled required>Select Genre</option>
                    <?php
                    while ($r = mysqli_fetch_array($genresResult)) {
                        $selected = ($bookDetails['bookCategory'] == $r['genrename']) ? 'selected' : '';
                        ?>
                        <option value="<?php echo $r['genrename']; ?>" <?php echo $selected; ?>><?php echo $r['genrename']; ?></option>
                    <?php
                    }
                    ?>
                </select>
            ...
            <div class="edit5">
                ...
                <input type="text" name="isbn" value="<?php echo $bookDetails['isbn']; ?>" disabled required>
            </div>  
